// _util.js – fælles helpers + demo/login-tilstand

const AK_KEYS = { users:'ak_users', settings:'ak_settings', cards:'ak_cards', session:'ak_session' };
const AK_FLAGS = { forceLogin:'ak_force_login' }; // "1" = login påkrævet
...
const defaultSettings = {
  ...
  // Default: ALLE felter påkrævede (kan slås fra i Admin)
...
function getSession(){
  const s=load(AK_KEYS.session,null); if(!s) return null;
  const now=Date.now();
  if(now - s.lastActivity > 15*60*1000){ localStorage.removeItem(AK_KEYS.session); return null; }
  return s;
}
...
function ensureDefaults(){
  ...
  if(!load(AK_KEYS.users,null)){
    sha256("1234").then(h=>save(AK_KEYS.users,[{id:crypto.randomUUID(),name:"Test Bruger",username:"test",pinHash:h,active:true}]));
  }
}

function startDemoSession(){
  const sess = { userId:'demo-user', role:'employee', username:'demo', name:'Demo Bruger', lastActivity:Date.now(), demo:true };
  save(AK_KEYS.session, sess);
  ensureDefaults();
  const cards = load(AK_KEYS.cards, []);
  if (!cards.some(c=>c.userId==='demo-user')) {
    cards.unshift({
      id: crypto.randomUUID(), userId: 'demo-user', date: yyyymmdd(new Date()),
      maskinNr: "DEMO-123", maskintimer: "100", km: "12", svejsningTimer: "0", elektroder: "0", diesel: "0",
      normalTimer: "7.5", overtimer: "0", arbejdetsArt: "Service",
      beskrivelse: "Demodata – så historik og visninger ikke er tomme.",
      consumables: [{name:"Olie",unit:"L",qty:1}], images: [], status: "sent", createdAt: Date.now(), updatedAt: Date.now()
    });
